...
import { months, oldAge, youngAge } from '../../../constants';
...
export default class ProfileExperienceForm extends Component {
  ...
  constructor(props){
    ...
    this.state = {
      jobs: null,
      errors: {},
      indexBlockFocus: null
    };
    let yearToday = new Date().getUTCFullYear();
    this.startYear = yearToday;
  ...
  componentWillMount(){
    this.setState({indexBlockFocus: 0});
  ...
  componentWillReceiveProps(newProps) {
    ...
    if (newProps.user && newProps.user.birthday) {
      ...
      this.endYear = birthday.getUTCFullYear() + youngAge;
    }
    if(newProps.errors) {
  ...
  _handlePositionsSelection(val, i) {
    ...
    this.setState({jobs: selectedJobs, indexBlockFocus: null});
  }
  ...
  _addExperienceBlock(e) {
    ...
    indexBlockFocus: this.state.jobs.length});
  }
  ...
  _handleSubmit(){
    ...
    this.setState({indexBlockFocus: null})
  }
            ...
            return (
              <div key={'experience-' + i} className={ 'formLayerRow  ' + (i % 2 == 1 ? 'evenrow' : '')}>
                <TextInput
                  ...
                  errors={errors[i] && errors[i].company}
                  inputRef={el => {
                    if (i == this.state.indexBlockFocus && el) {el.focus()}
                  }}
                />
                <Select
                  value={job.position}
                ...
                <TextArea
                    ...
                    errors={errors[i] && errors[i].responsibilitie}
                />
                <a
                    className='formlink'
