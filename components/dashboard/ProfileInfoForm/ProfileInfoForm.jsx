...
class ProfileInfoForm extends Component {
  ...
  componentWillReceiveProps(newProps){
    ...
    if(newProps.fetching || (Object.keys(newProps.errors).length != 0 && this.state.user)) return;
    let year = null;
    let month = null;
    let day = null;
  ...
  _setPhoneIsconfirmed(telNumber){
    ...
    if (newTel != oldTel) {
      ...
      if (this.props.user.phone_isconfirmed) {
        this.setState({phoneIsconfirmed: true})
      } else {
        this.setState({showButtonConfirm: true})
      }
    }
  ...
  _confirmMainPhone() {
    ...
    this.setState({showModalPinCode: true});
    ...
    return (
      <div>
        ...
        <div className="textInputTelephone">
          <label className='inputLabel inputLabel_default'>{gettext('Телефон')}</label>
          <ReactTelInput
            defaultCountry="ua"
          ...
          { user && this.state.phoneIsconfirmed &&
            <div className="reactInputChecked">
              <input
                ...
                checked="checked"
                disabled="disabled"
                title={gettext("Телефон підтверджено")}
              />
              <label htmlFor=""></label>
            </div>
          }
